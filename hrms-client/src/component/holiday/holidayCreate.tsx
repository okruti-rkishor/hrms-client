import { Divider, Layout, Table, Typography, message, Col } from "antd";
...
const HolidayCreateForm = ({ year, isFormDisabled, newData }: any) => {
  ...
  const onFinish = (values: any) => {
    console.log(values);
    dataProps = { ...values, name: values.name, type: values.type };
    ...
    if (values.date.length > 1) {
      ...
      payload.calender.endDate = dayjs(
        ...
        <div style={{ display: "flex", gap: "30px" }}>
          <Form.Item label={"Type"} name={"type"}>
            <Select placeholder={"Select the type of Holiday"}>
              <Select.Option value={"NATIONAL_HOLIDAY"}>
                National Holiday
              </Select.Option>
...
function HolidayCreate() {
  ...
  return (
    <Layout className="with-background">
      ...
      <div>
        <HolidayList  dataProps={dataProps}/>
      </div>
    </Layout>
  );
}
