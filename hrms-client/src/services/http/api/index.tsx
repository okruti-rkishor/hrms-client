...
export default {
    ...
    userMarkActive: (id: string, active: boolean) => rest.put(base + `user/${id}?active=${active}`, {}),
    ...
    documentUpload: (data: any) => rest.post(base + "s3/upload", {}, data, '', {'Content-Type': 'multipart/form-data'}),
    documentDelete: (id: string) => rest.del(base + `document/${id}`, {}, id),
    documentDeleteS3: (fileName: string) => rest.del(base + `s3/${fileName}`, {}, fileName),
    saveDocument : (data: any, id: string) =>  rest.post(base + "document/create",{},data,{'employee-id': id}),
    downloadDocument: (fileName: string) => rest.fetchData(base + "s3/download", {},{'fileName':fileName},{'Content-Type': 'application/octet-stream'}),
    getDocument:(id:string,data:any)=>rest.get(base+"document",{},{'employeeId':id,'type':data}),
    ...
    createEvent:(data:any) => rest.post(base + "event", {}, data, {}),
    editEvent: (data: any, id: string) => rest.put(base + `event/${id}`, {}, data),
    deleteEvent: (id: string) => rest.del(base + `event/${id}`, {},),
    getEvent: (id: string) => rest.get(base + `event/${id}`, {},),
    getAllEvent: () => rest.get(base + `event/all`, {},),
    ...
    getEntitlementByEmpLeaveType: (id: string, leaveType: string) => rest.get(base + `employee/${id}/leave-entitlements`, {}),
    getLeaveBalance: (id: string, leaveType?: string) => rest.get(base + `leave-balance`, {}, {
        "employee-id": id,
